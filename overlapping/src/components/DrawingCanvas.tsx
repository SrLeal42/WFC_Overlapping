...
export const DrawingCanvas: React.FC<Props> = ({ onGridChange }) => {
  ...
  const gridDataRef = useRef<C.ColorGrid>(
    Array(C.GRID_SIZE).fill(null).map(() => Array(C.GRID_SIZE).fill(C.DEFAULT_COLOR))
  );
  ...
  const [selectedColor, setSelectedColor] = useState<string>(C.ALL_COLORS[2]);
  ...
  const paintPixel = (mouseX: number, mouseY: number) => {
    ...
    const x = Math.floor((mouseX - rect.left) / C.PIXEL_SCALE);
    const y = Math.floor((mouseY - rect.top) / C.PIXEL_SCALE);
    ...
    if (x >= 0 && x < C.GRID_SIZE && y >= 0 && y < C.GRID_SIZE) {
      ...
      ctx.fillRect(x * C.PIXEL_SCALE, y * C.PIXEL_SCALE, C.PIXEL_SCALE, C.PIXEL_SCALE);
    }
  };
